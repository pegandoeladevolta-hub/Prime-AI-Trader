# Relatório de validação — PRIME AI TRADER 0.1.0
...
| 2. Interface responsiva | Grid central redimensionável, laterais fixas, painel esquerdo com rolagem e cards inferiores com rolagem horizontal. |
| 3. Binance | REST, paginação histórica de até 5.000 candles, símbolos USDT, book ticker, WebSocket e reconexão testada. |
| 4. Gráfico | Candles, volume, zoom, arraste, crosshair, OHLC, preço atual e overlays. |
...
| 9. Notícias | GDELT real, classificação local e bloqueio de risco. |
| 10. Forex | Provider Twelve Data, 10 pares iniciais, chave protegida e calendário Finnhub. |
...
- Resultado: **30 testes aprovados, 0 falhas**.
- Tempo da última execução: 49,544 segundos.
...
- parsing Binance, ausência de chave Forex e contrato de ações dos botões;
- troca de ativo/timeframe e reconexão WebSocket após falha transitória.

## Limite da validação neste ambiente

O código e os testes foram executados em ambiente Linux. O executável `.exe` e o instalador Inno Setup só podem ser compilados e assinados em um executor Windows; por isso, esta entrega contém o processo de build reproduzível, mas não afirma falsamente que um binário Windows foi testado aqui.
